...
`Condition.seoTitle` in `lib/conditions-data.ts` overrides the "X Treatment in Burlington" pattern in `generateConditionTitle`. Seventeen pages carry one: fourteen knee, hip and foot pages that already rank on Google page one for informational queries (worldwide, not Burlington), e.g. "Ankle Sprain: Grades, Recovery Time and Rehab Exercises", plus hip labral tears, sciatica and deep gluteal syndrome (added later on 2026-09-22 from the 90-day Search Console read: those pages drew impressions only on informational or wrong-page queries). The patellar compare page title uses the lay wording searchers type ("Patellar Tendonitis vs. Patellofemoral Pain Syndrome"). Leave the Burlington pattern on everything else; it is the local signal. Kareem does not offer shockwave therapy: the plantar fasciitis description and the site-search entry that claimed it were removed 2026-09-22; do not reintroduce it. The fluid-on-the-knee guide is "Suprapatellar Effusion: Causes, Symptoms and What to Do Next". IndexNow key file `public/a7f71a8cd17375152b88dbd64406713c.txt` must stay; after a content change, POST the changed URLs to `https://api.indexnow.org/indexnow` with that key (Bing, Copilot, ChatGPT search read Bing; Google ignores IndexNow). Both this site and endorphinshealth.com are in Bing Webmaster Tools under kareem.hassanein@gmail.com since 2026-09-22, imported from Search Console.
