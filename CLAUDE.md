...
His illustration set (19 engraved pieces, palette-corrected to navy/teal/gold/cream, 1122x1402) lives in `~/Documents/Websites/kinetikare-hero-review/plates/new/final/` with `web/` 900px WebP versions; the ChatGPT style prompt that made them is `plates/new/redo-for-chatgpt/PROMPT.md`. The integration sheet (`~/Documents/Websites/kinetikare-hero-review/integration/index.html`, served on 4185) shows the placements he has seen: a "Get back to it." row of six prints on navy after Care Journey (no captions, by his instruction), Care Journey drawn with sit-to-stand / step-ups / TRX rows / farmer carry, hub openers with the drawing on the page (knee running, hip golf, shoulder tennis, foot and ankle pickleball, elbow band rows), a closing "Getting back to it" band on every condition page (one print per category), and a three-print session strip on `/intake`. Print treatment: cream paper, a 1px gold hairline inset 12px, no shadow, no radius; on cream grounds no frame, the section takes the image's own paper colour.
