...
## Home hero: the seam (shipped 2026-09-15)

`components/sections/HeroSectionModern.tsx`. Navy text side on the left; on the right, Kareem's commissioned basketball-dunk illustration (`public/images/illustrations/basketball-dunk.webp`, 1400 px, from `~/Documents/Websites/kinetikare-hero-review/plates/new/final/basketball.png`) sitting on its own cream paper (`#f5efdf`), the two meeting at one gold diagonal seam (`clip-path: polygon(15% 0, 100% 0, 100% 100%, 0 100%)` on the art, an SVG line for the seam). No photograph, no frame, no review marquee, no illustration cutaways. The art starts at `top-[5.5rem]` under the header. On phones and tablets (below `xl`) the text comes first and the drawing sits below it in a 62vh block cut by the same seam along its top. The section carries `!bg-[#020617] !bg-none !py-0` because globals.css repaints every `<section>` and pads it (the padding showed as a navy strip under the phone artwork).

Chosen from a review sheet of five candidates in the same slot (`~/Documents/Websites/kinetikare-hero-review/integration/index.html`, served on 4185): tennis backhand, the dunk, pickleball, the layup, gardening. Kareem: "I like the basketball one yeah; the dunk thing". Rejected for the hero before that: the runner ("its not good"), golf ("not really my vibe"), and, earlier the same day, a canvas gait constellation ("I hate it"), Codex cutaway figures, an AI-adapted room, and every interactive idea ("I dont want it to be interactive in that way"). The rule that got us here: render candidates in a sheet, never push a hero live on "full creative control".

`components/Header.tsx`: the navy bar is now always on (`|| true` in the scrolled condition). It used to go transparent at the top of the home page over the photo; white links over cream paper were unreadable.

Two SVG traps met here: Framer's `pathLength` animation on a `<line>` inside a `preserveAspectRatio="none"` viewBox with `vector-effect: non-scaling-stroke` renders as dashes (animate opacity instead); and the seam line must be a plain `<line>`, not a path with dasharray.

The rest of the illustration set (17 more finals in `plates/new/final/` and the five untouched originals) is not on the site yet. The sheet also shows where they go: a "Get back to it." row of six prints on navy after Care Journey (no captions, by his instruction), Care Journey drawn with sit-to-stand / step-ups / TRX rows / farmer carry, hub openers with the drawing on the page (knee running, hip golf, shoulder tennis, foot and ankle pickleball, elbow band rows), a closing "Getting back to it" band on every condition page (one print per category), and a three-print session strip on `/intake`. Build those only after he points at them.
