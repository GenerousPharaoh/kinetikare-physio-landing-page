...
## Home hero (state as of 2026-09-15)

`components/sections/HeroSectionModern.tsx`: the treatment-room split (newer Endorphins photo `/images/endorphins-treatment-room.webp` on the right behind the diagonal clip, text left), with the vertical review marquee **removed** at Kareem's request; the mobile trust line (5.0 on Google, 31 reviews) and the "Accepting new patients" badge remain. Review count surfaces: `GoogleReviews.tsx`, this hero's mobile trust line, and the two `/intake` hits.

Two full redesigns were shipped and reverted the same day, both under "full creative control": (1) a live motion-capture gait constellation drawn on a canvas over the dimmed room ("nah I hate it; its so stupid"); (2) before that, two Codex-generated figures with the joint anatomy opened ("Im not a fan of these designs"). Also refused as options: an AI-adapted room, a photo-only hero, a typography-only hero, abstract AI artwork. Do not re-propose any of these. Next hero work should be shown to him as rendered options in a review sheet before anything ships (memory `feedback_show_rendered_design_options`), not pushed live.
