...
`GoogleAnalytics.tsx` sets the `ga-disable-<id>` opt-out flags when the hostname is not kinetikarephysio.com (local builds, Vercel previews) or `navigator.webdriver` is true (headless browsers), so those hits never reach GA4 or Ads (added 2026-09-22 after 9 localhost sessions and a large Singapore headless-Chrome share showed up in the property).
...
## Home hero (state as of 2026-09-15, evening)

`components/sections/HeroSectionModern.tsx` is the **treatment-room split**: the newer Endorphins photo (`/images/endorphins-treatment-room.webp`) on the right behind the diagonal clip at scale 1 with a faint drift, text left, no review marquee, the "Accepting new patients" badge at `right-[15%]`. This is commit `0b18b91`'s file, restored after the illustrated hero.

Everything tried for the hero on 2026-09-14/15 and rejected, in order: a canvas gait constellation ("I hate it"), Codex cutaway figures, an AI-adapted room, anything interactive ("I dont want it to be interactive in that way"), the runner and golf illustrations in the seam layout, and finally the basketball dunk in the seam layout, which he picked from a sheet, saw live, and reversed ("the basketball stuff isnt good; I would switch it back to the treatment room for that section"). **The hero stays the room photo. His illustrations go elsewhere on the site.** Header: the navy bar is always on (`|| true` in the scrolled condition), kept because it does no harm and the transparent state only existed for the photo.

## Kareem's illustrations on the site (shipped 2026-09-15)

His set (19 engraved pieces in one palette: navy, dark teal, gold, cream; 1122x1402, 4:5) lives in `~/Documents/Websites/kinetikare-hero-review/plates/new/final/`, with the ChatGPT style prompt that made them at `plates/new/redo-for-chatgpt/PROMPT.md` (use it for any new piece: engraved finish, faces turned away, no logos, portrait 4:5). The site carries 900px WebP copies in `public/images/illustrations/` (1.9 MB total).

- `lib/illustrations.ts` is the registry: each entry has its `paper` colour (sampled from the file's corners) so a container can match it and the drawing sits on the page with no edge; two are dark (farmer carry, band rotations). It also holds every placement mapping. **Add or move an illustration there, not in components.**
- `components/Print.tsx`: the print mount, cream paper, one gold hairline inset 12px, no shadow, no radius (the anatomy medallions' mount, so both sets read as one library). Decorative by default (empty alt).
- Placements, all from the integration sheet he reviewed: `GetBackSection` (home, after Care Journey: six life prints on navy under "Get back to it." alone: **no captions and no subline**, both cut at his instruction; "The goal is the activity, not the exercise" was "SO dumb"); `CareJourneySection` (the four stages drawn: sit-to-stand, step-ups, TRX rows, farmer carry, number badge on the corner of each print, print at the left on phones); the five regional hubs (the drawing on the page, section background set to the image's paper: knee running, hip golf, shoulder tennis, foot and ankle pickleball, elbow gardening); the closing band on all 61 condition pages ("Getting back to it", one print per category via `CATEGORY_PRINT`, `Contact` now goes to `/contact`); a three-print "What a session looks like" strip on `/intake` before the clinic block.
- **Repetition rule (2026-09-16).** Kareem: the prints row was "so repetitive with the woman being there three times stacked on top of each other". Tennis, running and pickleball are the same figure (ponytail, white top, navy bottoms) and the phone grid stacked all three in one column. `GET_BACK_ROW` is now tennis, golf, gardening, walking, yoga, running (pickleball out of the row, it duplicates tennis), ordered so no two similar figures sit beside or above each other at any breakpoint, and `GetBackSection` hangs two print sizes bottom-aligned (phone: three rows of two with the large print alternating sides; tablet: three across; desktop: large, small, large, small on one shelf) instead of a uniform grid. Apply the same check to any new print grid: the same figure is never adjacent and never in one column.
- Not used anywhere: basketball dunk, basketball defence, side lunge, single-leg RDL. Kettlebell squat only on `/intake`; pickleball only on the foot-ankle hub and the elbow-wrist-hand condition band. Band rows is still being redrawn.
...
`context/PerformanceContext.tsx` wraps the app in `LazyMotion features={domAnimation}` and components import `m as motion` from framer-motion (31 files). New components must do the same; importing `motion` pulls the full runtime back into the bundle. `ConditionsPageClient.tsx` keeps `motion` because it uses `layoutId`. Both heroes (`HeroSectionModern`, `IntakeLandingPage`) enter with CSS keyframes (`.hero-rise` in `app/globals.css`, `.intake-rise` inline) instead of Framer variants, so the H1 is never server-rendered at `opacity:0`. `@heroicons` has its own cacheGroup in `next.config.js`. `experimental.inlineCss` is on (since 2026-09-22): the CSS ships inside each HTML page as `<style>` instead of a render-blocking `<link>`, which removed one round trip from first paint; client-side navigation keeps working (verified headless). The header logo is `priority` so it is preloaded rather than lazy-loaded (Lighthouse had picked the lazy logo as a late LCP element). **Hero fades start at opacity 0.01, never 0** (`hero-rise`, `hero-image-in`, `intake-rise`): Chrome records an opacity-0 element as painted only at the next main-thread paint after it becomes visible, which on a slow phone is after hydration, so the headline measured as a 2.2 s LCP while it visibly faded in at 0.9 s; at 0.01 it counts from first paint (measured cold and throttled: /intake LCP 2.17 s -> 0.66 s, home 2.13 s -> 0.70 s). Any new entrance animation on above-the-fold content follows the same rule. Probes and the post-build smoke test live in `~/Documents/Physio/KinetiKare marketing/perf/` (README there); run `smoke.mjs` against the local production server after every build. `ANALYZE=true npm run build` writes a bundle report to `analyze/` (gitignored). Measured 2026-09-22 (gzip, modern browsers; the `nomodule` polyfills never download): /intake JS 240.6 -> 233.9 KB, knee hub 195.7 -> 184.4 KB, framer chunk 36.5 -> 23.4 KB. Known quirk: Next's client-reference manifest attaches the home page chunk to /intake, /treatments/* and the legal pages because they share client modules; a `nextClient` cacheGroup made it worse and was reverted.

## Condition page titles (2026-09-22, late)

`Condition.seoTitle` in `lib/conditions-data.ts` overrides the "X Treatment in Burlington" pattern in `generateConditionTitle`. Seventeen knee, hip and foot pages (plus sciatica) carry one, and since the night of 2026-09-22 (commit 774d105) every one is **plain and keeps Burlington**: "[Condition]: Symptoms and Treatment in Burlington" or the nearest plain form that fits 60 characters (e.g. "Ankle Sprain: Recovery Time and Treatment in Burlington", "Patellar Tendonitis (Jumper's Knee) Treatment in Burlington"). Descriptions say what the page covers, name Burlington, and end on direct billing / no referral / evening hours where they fit. History, so it is not repeated: on 2026-09-22 these pages briefly carried question-style titles aimed at their worldwide informational impressions ("Sciatica: Symptoms, Causes and What Helps It Settle", "Patellofemoral Pain: Why Your Knee Hurts When Bending"). Kareem called them stupid, and the goal was wrong: those impressions are US and UK searchers who cannot book, while the business is Burlington searches. No folksy verbs ("settle", "what helps", "the rehab that works"), no lecturing asides, no efficacy claims in titles or descriptions. The patellar compare page title uses the lay wording searchers type ("Patellar Tendonitis vs. Patellofemoral Pain Syndrome: How to Tell"). Kareem does not offer shockwave therapy: the plantar fasciitis description and the site-search entry that claimed it were removed 2026-09-22; do not reintroduce it. The fluid-on-the-knee guide is "Suprapatellar Effusion (Fluid on the Knee) in Burlington". IndexNow key file `public/a7f71a8cd17375152b88dbd64406713c.txt` must stay; after a content change, POST the changed URLs to `https://api.indexnow.org/indexnow` with that key (Bing, Copilot, ChatGPT search read Bing; Google ignores IndexNow). Both this site and endorphinshealth.com are in Bing Webmaster Tools under kareem.hassanein@gmail.com since 2026-09-22, imported from Search Console.
