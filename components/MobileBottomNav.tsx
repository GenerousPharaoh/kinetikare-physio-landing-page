...
import { HomeIcon, Squares2X2Icon, CalendarDaysIcon, UserIcon } from '@heroicons/react/24/outline';
...
const navItems = [
  ...
  { href: '/services', label: 'Services', icon: Squares2X2Icon },
...
export default function MobileBottomNav() {
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 h-16 bg-white border-t border-neutral-200 shadow-lg z-40 flex items-center justify-around">
      {navItems.map((item) => (
        <Link
          key={item.label}
          href={item.href}
