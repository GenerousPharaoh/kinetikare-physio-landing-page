            ...
            <motion.div initial="hidden" animate="visible" variants={reduced ? undefined : stagger} style={{ display: 'grid', width: '100%', alignItems: 'center', gap: 'clamp(3rem, 6vw, 5rem)', gridTemplateColumns: '1fr' }} className="lg:!grid-cols-[1fr_340px]">
              ...
              <motion.div style={{ paddingTop: 'clamp(0rem, 4vh, 3rem)' }}>
                ...
                <motion.div variants={up} style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 16, paddingTop: 8, paddingBottom: 16, borderTop: `1px solid ${c.stone100}` }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 13, color: c.textLight }}>from 31 reviews</span>
                  </div>
              ...
              <motion.div className="hidden lg:block" initial={reduced ? false : { opacity: 0.01, x: 40 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 1.2, delay: 0.25, ease: [0.22, 1, 0.36, 1] }} style={{ position: 'relative', zIndex: 10 }}>
                <motion.div style={{ position: 'relative', y: reduced ? 0 : photoY, overflow: 'hidden', aspectRatio: '3 / 4', borderRadius: 6 }}>
                  <Image src="/images/kareem-hassanein-portrait.webp" alt="Kareem Hassanein, Registered Physiotherapist in Burlington" width={1326} height={1187} priority quality={82} sizes="(min-width: 1024px) 510px, 0px" style={{ width: '100%', height: '100%', display: 'block', objectFit: 'cover', objectPosition: 'top center' }} />
        ...
        <div ref={reviewRef} style={{ position: 'relative', overflow: 'hidden' }}>
          ...
          <div style={{ position: 'relative', maxWidth: 780, margin: '0 auto', padding: 'clamp(7rem, 16vw, 14rem) clamp(1.5rem, 5vw, 4rem)', textAlign: 'center' }}>
            <Reveal from="scale">
              ...
              <p style={{ fontSize: 12, fontWeight: 600, letterSpacing: '0.2em', textTransform: 'uppercase', color: c.goldBright, marginBottom: 48 }}>5.0 from 31 Google Reviews</p>
