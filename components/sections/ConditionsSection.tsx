...
const ConditionsSection = () => {
  ...
  return (
    <section id="conditions" className="relative overflow-hidden py-20 bg-primary-900 text-white">
      {/* Decorative elements - replaced with subtle geometric shapes */}
      <div className="absolute top-0 right-0 w-96 h-96 rounded-full bg-primary-800/30 blur-xl"></div>
      <div className="absolute bottom-0 left-0 w-96 h-96 rounded-full bg-primary-800/20 blur-xl"></div>
      ...
      <div className="container mx-auto px-4 md:px-6 relative z-10">
        ...
        >
          <h2 className="text-3xl md:text-4xl font-bold text-white mb-4">Conditions I Treat</h2>
        ...
        {/* Tabs */}
        <div className="flex flex-wrap justify-center mb-12 border-b border-primary-700">
          <button 
            onClick={() => setActiveTab('common')}
            className={`px-6 py-3 text-lg font-medium transition flex items-center 
              ${activeTab === 'common' 
                ? 'text-white border-b-2 border-primary-300' 
        ...
        >
          {conditions[activeTab as keyof typeof conditions].map((condition, index) => (
            <motion.div 
              key={index} 
              variants={item}
              className="bg-primary-800 rounded-lg overflow-hidden shadow-xl hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 group p-5"
            >
              <div className="">
                <h3 className="text-xl font-bold text-white mb-2">{condition.title}</h3>
                <p className="text-primary-100">{condition.description}</p>
              </div>
            </motion.div>
          ))}
        </motion.div>
